...
export const SponsorMarquee = () => {
    return (
        <section className="marquee-section">
            ...
            <div className="marquee-container reveal reveal-delay-200">
                <div className="marquee-track">
                    {/* Double the list for seamless infinite loop */}
                    {[...SPONSORS, ...SPONSORS].map((sponsor, index) => (
                        <div className="marquee-item" key={`${sponsor.id}-${index}`}>
                            <img src={sponsor.imgUrl} alt={`${sponsor.name} logo`} className="sponsor-logo" />
                        </div>
                    ))}
                </div>
            </div>
